...
openssl smime -encrypt -aes-256-cbc -in "$MASTER_KEY_FILE" -outform DER "$CERT_FILE" | base64 -w 0 > encrypted_master_key.b64
ENCRYPTED_MASTER_KEY=$(cat encrypted_master_key.b64)
...
if [ $? -ne 0 ]; then
    ...
    rm "$CERT_FILE" "$CA_CERT_FILE" "$MASTER_KEY_FILE" encrypted_master_key.b64
    exit 1
fi
...
echo "$ENCRYPTED_SAMPLE_MESSAGE" | base64 -d > encrypted_sample_message.der

# Decrypt the sample message
DECRYPTED_SAMPLE_MESSAGE=$(openssl enc -d -aes-256-cbc -pbkdf2 -in encrypted_sample_message.der -k "$MASTER_KEY")
if [ "$DECRYPTED_SAMPLE_MESSAGE" != "Hi server, please encrypt me and send to client!" ]; then
    echo "Server symmetric encryption using the exchanged master-key has failed."
    rm "$CERT_FILE" "$CA_CERT_FILE" "$MASTER_KEY_FILE" encrypted_master_key.b64 encrypted_sample_message.der
    exit 6
fi
